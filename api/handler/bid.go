...
func (h *Handler) CreateBid(c *gin.Context) {
	...
	if err != nil {
		h.log.Error(fn, err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "Failed to create bid"})
		return
	}
...
func (h *Handler) GetBidListByTenderId(c *gin.Context) {
	...
	if err != nil {
		if err == customerrors.ErrBidNotFound {
			c.AbortWithStatusJSON(http.StatusNotFound, gin.H{
				"reason": "У вас нет предложений.",
			})
			return
		}
		h.log.Error(fn, err)
...
func (h *Handler) GetBidStatus(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, customerrors.ErrBidNotFound) {
			h.log.Error(fn, err)
			c.AbortWithStatusJSON(http.StatusNotFound, gin.H{"reason": "Bid not found"})
			return
		}
		h.log.Error(fn, err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"reason": "Failed to get bid status"})
		return
	}
...
func (h *Handler) ChangeBidStatus(c *gin.Context) {
	...
	if err != nil {
		h.log.Error(fn, err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "Failed to change bid status"})
		return
	}
...
func (h *Handler) ChangeBidById(c *gin.Context) {
	...
	if err != nil {
		h.log.Error(fn, err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "Failed to change bid"})
		return
	}
...
func (h *Handler) SendDesicionByBid(c *gin.Context) {
	...
	if err != nil {
		h.log.Error(fn, err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "Failed to send decision status"})
		return
	}
...
func (h *Handler) SendFeedbackByBid(c *gin.Context) {
	...
	if err != nil {
		h.log.Error(fn, err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "Failed to send feedback"})
		return
	}
...
func (h *Handler) GetFeedbacksByTenderAndAuthor(c *gin.Context) {
	...
	if err != nil {
		h.log.Error(fn, err.Error())
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "Failed to get feedbacks"})
		return
	}
...
func (h *Handler) RollbackBid(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, customerrors.ErrBidNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"reason": "Bid or version not found"})
		} else {
