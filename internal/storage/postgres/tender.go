...
func (t *tenderRepo) GetMyTenders(limit, offset int, username uuid.UUID) ([]models.Tender, error) {
	const fn = "storage.tenderRepo.GetMyTenders"

	query := `SELECT id, name, description, status, service_type, version, created_at
				FROM tenders where responsible_id = $1 LIMIT $2 OFFSET $3`
	rows, err := t.db.Query(query, username, limit, offset)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("%s: %w", fn, customerrors.ErrTenderNotFound)
		}
...
func (t *tenderRepo) GetTenderStatusById(tenderId uuid.UUID, userId string) (string, error) {
	...
	if userId == uuid.Nil.String() {
		...
		query = `SELECT status
				FROM tenders where id = $1 and responsible_id = $2`
		rows, err = t.db.Query(query, tenderId, userId)
	}
